  ...
  arrayReceivedOnEndpoint.forEach(data => {
    switch (data.serviceType) {
      case HDR_SERVICES_TYPE.health:
        processedMessages.push({ serviceType: 'HEALTH', mac: data.mac, rssi: data.rssi, ...processHealth(data.raw, data.time) })
        break;
      case HDR_SERVICES_TYPE.temp:
        processedMessages.push({ serviceType: 'TEMP', mac: data.mac, rssi: data.rssi, ...processTemp(data.raw, data.time) })
        break;
      case HDR_SERVICES_TYPE.rmms:
        processedMessages.push({ serviceType: 'RMMS', mac: data.mac, rssi: data.rssi, ...processRMMS(data.raw, data.time) })
        break;
      case HDR_SERVICES_TYPE.rms2:
        processedMessages.push({ serviceType: 'RMS2', mac: data.mac, rssi: data.rssi, ...processRMS2(data.raw, data.time) })
        break;
      case HDR_SERVICES_TYPE.fft:
        processedMessages.push({ serviceType: 'FFT', mac: data.mac, rssi: data.rssi, ...processFFT(data.raw, data.time) })
        break;
